...
class WaymoDataset(DatasetTemplate):
    ...
    def get_lidar(self, sequence_name, sample_idx):
        ...
        beam_labels_file = self.data_path / sequence_name / ('%04d.beam_labels.npy' % sample_idx)
        beam_labels = np.load(beam_labels_file).astype(np.int)  # (N, )

        points_all, NLZ_flag = point_features[:, 0:5], point_features[:, 5]
        points_all = points_all[NLZ_flag == -1]
        beam_labels = beam_labels[NLZ_flag == -1]
        points_all[:, 3] = np.tanh(points_all[:, 3])
        return points_all, beam_labels
    ...
    def __getitem__(self, index):
        ...
        points, beam_labels = self.get_lidar(sequence_name, sample_idx)
